...
class TiRexForecaster(BaseForecaster):
    ...
    _tags = {
        ...
        "python_dependencies": ["tirex", "torch"],
        "tests:vm": True,
    }

    # todo: add any hyper-parameters and components to constructor
    def __init__(self, model="NX-AI/TiRex"):
    ...
    def _fit(self, y, X, fh):
        ...
        self.model_ = load_model(self.model, device="cpu")
    ...
    def _predict(self, fh, X):
        ...
        forecast_np = forecast.reshape(-1, forecast.shape[-1])[0]

        index = fh.to_absolute(self.cutoff).to_pandas()

        return pd.Series(
            forecast_np[0], index=index, name=(y.name if hasattr(y, "name") else None)
        )
